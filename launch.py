...
parser.add_argument('file', default='flyingBlender.py', nargs='?', help='The path to a configuration file which describes what the pipeline should do or a python file which uses BlenderProc via the API.')
...
parser.add_argument('--blender-install-path', dest='blender_install_path', default="blender", help="Set path where blender should be installed. If None is given, /home_local/<env:USER>/blender/ is used per default. This argument is ignored if it is specified in the given YAML config.")
...
if args.debug:
    ...
    if not args.batch_process:
        commands = [blender_run_path, "--background", "--python-use-system-env", "--python-exit-code", "2", "--python", path_src_run, "--", args.file, temp_dir] + args.args
        print(f"Subprocess running:{(' '.join(commands))}")
        p = subprocess.Popen(commands, env=dict(os.environ, PYTHONPATH=os.getcwd(), PYTHONNOUSERSITE="1"), cwd=repo_root_directory)
    else:  # Pass the index file path containing placeholder args for all input combinations (cam, house, output path)
        commands = [blender_run_path, "--background", "--python-use-system-env", "--python-exit-code", "2", "--python", path_src_run, "--",  args.file, temp_dir, "--batch-process", args.batch_process]
        print(f"Subprocess running:{(' '.join(commands))}")
        p = subprocess.Popen(commands, env=dict(os.environ, PYTHONPATH=os.getcwd(), PYTHONNOUSERSITE="1"), cwd=repo_root_directory)
...
from make_events import post_events_after_blender

try:
    p.wait()
    post_events_after_blender(f"{temp_dir}/.xpf_item_dir_path.txt")
