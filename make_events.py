...
def convert_events(item_dir, preview=True, fps=60):
    ...
    os.system(f"ffmpeg -framerate 30 -pattern_type glob -i '{item_dir}/events/evt_view_*.png'"
                f" -c:v libx264 -pix_fmt yuv420p {item_dir}/events.mp4 -y")

def post_events_after_blender(item_dir_p='.xpf_item_dir_path.txt'):
    with open(item_dir_p, 'rt') as f:
        item_dir = f.read()
    fps = 60
    convert_events(item_dir, False, fps=fps)
    print(item_dir)
    eval_evt_flow(item_dir, fps=fps)
    return item_dir
...
def eval_evt_flow(item_dir, fps):
    rgb_dir = f"{item_dir}/rgb"
    events_dir = f"{item_dir}/events"
    evt_flow_dir = f"{item_dir}/evt_flow"
    ...
    avg_EPE, EPE_lt_3, Fl = absolute_EPE(evt_flow_speed_range, events_end2end_flow, fps)
    # import pdb; pdb.set_trace()
    var_compare = str(f"Varances are {var_evt:.4f}(before) / {var_warp:.4f}(after) / {var_e2e:.4f}(e2e).\n"
            f"Downscale rate {var_evt/var_warp:.4f}x(after warp) / {var_evt/var_e2e:.4f}x(end 2 end)\n"
            f"The optical flow accuracy metrics are as below:\n"
            f"\tavg_EPE \t: {avg_EPE:.4f}\n"
            f"\tEPE_lt_3 \t: {EPE_lt_3:.4f}\n"
            f"\tFl \t\t: {Fl:.4f}\n")
    print(var_compare)
    with open(f'{evt_img_dir}/var_compare.txt', 'wt') as f:
        f.write(var_compare)

def absolute_EPE(flow_est, flow_gt, fps):   # flow speed input
    # 光流终点误差的绝对值 End point Error
    flow_est /= fps # 把speed 转换为 一帧之间的位移
    flow_gt /= fps
    def euler_dist(x_2d):
        return np.sqrt((x_2d**2).sum(1))
    abs_EPE = euler_dist(flow_est - flow_gt)
    avg_EPE = abs_EPE.mean()
    abs_EPE_lt_3 = abs_EPE > 3
    EPE_lt_3 = abs_EPE_lt_3.sum() / len(flow_gt)
    rel_EPE_lt_0_05 = (abs_EPE / (euler_dist(flow_gt)+1e-15)) > 0.05
    Fl = (abs_EPE_lt_3 & rel_EPE_lt_0_05).sum() / len(flow_gt)
    return avg_EPE, EPE_lt_3, Fl
